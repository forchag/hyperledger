...
% Note: merged from earlier duplicate sections.
...
We use: (i) latency $L$ (end‑to‑end), (ii) jitter $J=\sqrt{\mathrm{Var}[D]}$ for delay $D$, (iii) reliability $R=\Pr\{D\le D_{\max}\}$, (iv) throughput $T$ (tx/s), and (v) availability $A$ (uptime fraction meeting SLOs).

\paragraph{Protocol models.}
\textbf{PBFT} with $n=3f+1$ replicas tolerates $f$ Byzantine faults; commit requires pre‑prepare/prepare/commit rounds with $O(n^2)$ messages. A latency proxy is
\[
...
\textbf{RAFT} (crash‑fault tolerant) elects a leader and commits on majority ($n=2f+1$). Latency is dominated by leader→followers replication and fsync:
\[
C_{\mathrm{RAFT}} \approx \mathrm{RTT}_{\mathrm{leader}} + \tau_{\mathrm{fsync}} + \tau_{\mathrm{batch}}.
\]  
\textbf{PoS committees} (e.g., VRF‑selected) reduce validator sets to $k\ll n$, cutting network quadratic costs. Let $C_{\mathrm{PoS}}\approx \mathrm{RTT}_{\text{committee}} + \tau_{\mathrm{agg}}$ (aggregation/threshold signature).
...
\]
where $B_i$ is the batch size. For M/M/1, $W_i=1/(\mu_i-\lambda_i)$. \emph{Batching trade‑off:} increasing $B_i$ reduces per‑tx overhead in $C_i$ (higher $\mu_i$) but inflates $W_i$ at low load. Choose $B_i^\star$ by minimizing $L_i(W_i+C_i)$ subject to SLOs (deadline $D_{\max}$, jitter cap $J_{\max}$).
...
% Note: merged from earlier duplicate sections.
\section{Comparison Tables (Merged Pool)}
\begin{table}[htbp]
